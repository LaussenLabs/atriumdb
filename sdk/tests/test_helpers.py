...
from atriumdb.adb_functions import group_sorted_block_list, get_interval_list_from_ordered_timestamps, \
    group_headers_by_scale_factor, group_headers_by_scale_factor_all
...
def test_group_headers_by_scale_factor_all():
    # Test case 1: All headers have the same scale factors
    headers1 = [
        MockHeader(1.0, 2.0, 3),
        MockHeader(1.0, 2.0, 2),
        MockHeader(1.0, 2.0, 4),
    ]
    times_array1 = np.array(list(range(9)))  # times from 0 to 8
    values_array1 = np.array(list(range(100, 109)))  # values from 100 to 108
    expected_groups1 = [
        (headers1, times_array1, values_array1)
    ]
    ...
    # Test case 2: Headers with different scale factors
    headers2 = [
        MockHeader(1.0, 2.0, 3),  # header 0
        MockHeader(1.1, 2.0, 2),  # header 1
        MockHeader(1.0, 2.0, 4),  # header 2
    ]
    times_array2 = np.array(list(range(9)))
    values_array2 = np.array(list(range(200, 209)))
    expected_groups2 = [
        ([headers2[0], headers2[2]], np.array([0,1,2,5,6,7,8]), np.array([200,201,202,205,206,207,208])),
        ([headers2[1]], np.array([3,4]), np.array([203,204])),
    ]

    # Test case 3: Empty headers list
    headers3 = []
    times_array3 = np.array([])
    values_array3 = np.array([])
    expected_groups3 = []
    ...
    headers4 = [
        ...
        MockHeader(1.1, 2.0, 2),  # header 1
        MockHeader(1.0, 2.0, 4),  # header 2
        MockHeader(1.0, 2.0, 1),  # header 3
    ]
    times_array4 = np.array(list(range(10)))
    values_array4 = np.array(list(range(300, 310)))
    expected_groups4 = [
        ([headers4[0], headers4[2], headers4[3]], np.array([0,1,2,5,6,7,8,9]), np.array([300,301,302,305,306,307,308,309])),
        ([headers4[1]], np.array([3,4]), np.array([303,304])),
    ]

    # Test case 5: All headers have different scale factors
    headers5 = [
        MockHeader(1.0, 2.0, 1),  # header 0
        MockHeader(1.1, 2.1, 2),  # header 1
        MockHeader(1.2, 2.2, 3),  # header 2
    ]
    times_array5 = np.array(list(range(6)))
    values_array5 = np.array(list(range(400, 406)))
    expected_groups5 = [
        ([headers5[0]], np.array([0]), np.array([400])),
        ([headers5[1]], np.array([1,2]), np.array([401,402])),
        ([headers5[2]], np.array([3,4,5]), np.array([403,404,405])),
    ]

    # Test case 6: Only one header
    headers6 = [MockHeader(1.0, 2.0, 3)]
    times_array6 = np.array(list(range(3)))
    values_array6 = np.array(list(range(500, 503)))
    expected_groups6 = [
        (headers6, times_array6, values_array6)
    ]
    ...
    times_array7 = np.array([])
    values_array7 = np.array([])
    expected_groups7 = [
        (headers7, np.array([]), np.array([]))
    ]

    # Test case 8: Mixed headers with non-consecutive identical scale factors
    headers8 = [
        MockHeader(1.0, 2.0, 2),  # header 0
        MockHeader(1.1, 2.1, 1),  # header 1
        MockHeader(1.0, 2.0, 3),  # header 2
        MockHeader(1.2, 2.2, 2),  # header 3
        MockHeader(1.1, 2.1, 2),  # header 4
        MockHeader(1.0, 2.0, 1),  # header 5
    ]
    times_array8 = np.array(list(range(11)))  # times from 0 to 10
    values_array8 = np.array(list(range(600, 611)))  # values from 600 to 610
    expected_groups8 = [
        ([headers8[0], headers8[2], headers8[5]], np.array([0,1,3,4,5,10]), np.array([600,601,603,604,605,610])),
        ([headers8[1], headers8[4]], np.array([2,8,9]), np.array([602,608,609])),
        ([headers8[3]], np.array([6,7]), np.array([606,607])),
    ]
    ...
    test_cases = [
        ...
        (headers6, times_array6, values_array6, expected_groups6),
        (headers7, times_array7, values_array7, expected_groups7),
        (headers8, times_array8, values_array8, expected_groups8),
    ]

    # Run tests
    for idx, (headers, times_array, values_array, expected_groups) in enumerate(test_cases, 1):
        result = list(group_headers_by_scale_factor_all(headers, times_array, values_array))
        assert len(result) == len(expected_groups), f"Test case {idx}: Number of groups does not match expected."

        for i, (group, times_slice, values_slice) in enumerate(result):
            expected_group, expected_times, expected_values = expected_groups[i]
            assert group == expected_group, f"Test case {idx}, group {i+1}: Headers do not match expected."
            assert np.array_equal(times_slice, expected_times), f"Test case {idx}, group {i+1}: Times slice does not match expected."
            assert np.array_equal(values_slice, expected_values), f"Test case {idx}, group {i+1}: Values slice does not match expected."
